<?php
    ...
    $columns = array_merge($allusernamefields, $extracolumns, array('status', 'city', 'country', 'lastaccess'));
    ...
    if (!$users) {
        ...
        foreach ($users as $user) {
            ...
            $row = array();
            $row[] = profile_display_custom_field($user->id, 'noinduk');
            $row[] = "<a href=\"../user/view.php?id=$user->id&amp;course=$site->id\">$fullname</a>";
            foreach ($extracolumns as $field) {
                $row[] = $user->{$field};
            }
            $row[] = profile_display_custom_field($user->id, 'status');
            $row[] = $user->city;
            $row[] = $user->country;
            $row[] = $strlastaccess;
            if ($user->suspended) {
